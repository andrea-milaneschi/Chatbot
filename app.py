...
df = load_local()
if df is None:
    st.info("Carica i file Excel del database qui sotto.")
    uploaded = st.file_uploader(
        "Seleziona uno o più file .xls",
        type=["xls", "xlsx"],
    ...
    if uploaded:
        ...
        st.success(f"Database caricato: {len(df)} record.")
    else:
        st.stop()
else:
    st.caption(f"Database locale caricato — {len(df)} record totali.")
